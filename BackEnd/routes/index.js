...
const { bookSlot, verifyPayment, approveBooking, showBooked, showBooked1, getUserProductBookings, cancelSlot, getPastBookings, setRenovationStatus } = require("../controller/bookingcontroller");
const { bookTournamentSlot, getAllTournaments, getTournamentById, setTournamentStatus, getUserTournamentBookings } = require("../controller/tournamentbookingcontroller");
...
router.get('/user-tournament-bookings/:product_id/:user_id', getUserTournamentBookings);
router.post('/cancel-slot', cancelSlot);
router.get('/past-bookings', getPastBookings);
router.post('/set-renovation-status/:productId', setRenovationStatus);
router.post('/set-tournament-status/:productId', setTournamentStatus);

router.post("/book-tournament-slot", bookTournamentSlot);
