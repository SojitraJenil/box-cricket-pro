...
export const updateProductRenovation = async (productId: string) => {
  try {
    const response = await axiosInstance.post(`/set-renovation-status/${productId}`, {}, {
      headers: {
...
export const updateProductTournament = async (productId: string) => {
  try {
    const response = await axiosInstance.post(`/set-tournament-status/${productId}`, {}, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
